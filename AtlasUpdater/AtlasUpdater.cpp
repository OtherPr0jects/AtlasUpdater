...
#define ATLAS_VERSION_FILE_NAME "Atlas.ver"
...
std::string getExecutablePath() { // Location of the executable file. NOT the current working directory.
    ...
    replace(stringPath, "AtlasUpdater.exe", "");
    return stringPath;
}
...
int main() {
    ...
    curl_easy_setopt(request, CURLOPT_URL, "https://api.github.com/repos/OtherPr0jects/Atlas/tags");
    ...
    if (latestVersion != currentVersion) { // Update found.
        ...
        fileDownloadLink << "https://github.com/OtherPr0jects/Atlas/releases/download/" << latestVersion << "/Atlas.zip";
        
        FILE* zipFile = fopen((path + "Atlas.zip").c_str(), "wb");
        ...
        HZIP openedZip = OpenZip((path + "Atlas.zip").c_str(), 0);
        ZIPENTRY zipInfo; 
        ...
        CloseZip(openedZip);
        remove((path + "Atlas.zip").c_str());

        std::cout << "Done." << std::endl;
    }

    std::cout << "Up to date! Enjoy Atlas." << std::endl;
